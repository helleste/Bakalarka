...
\textit{\uv{Jsou příliš finančně výhodné, než aby se daly ignorovat,}} říká Dominique Hazael-Massieux, vedoucí sekce Mobile Web Initiative v rámci konsorcia W3C \cite{bii_hybrid_apps_report}.
...
\\ \\
\textit{\uv{Společnosti stále častěji zjišťují, že musí podporovat více platforem, zejména když ‚BYOD‘ přístup nabývá na síle.} Gartner \cite{gartner_says_50}}
...
\textit{\uv{Kvalita uživatelského rozhraní, kterého může vývojář vývojář dotáhnout v nativní aplikaci, jednoduše nestojí za extra vynaložené prostředky oproti hybridní cestě, jež umožňuje vytvořit uživatelské rozhraní dostatečné kvality,}} říká Ron Perry, technický ředitel firmy Worklight \cite{hybrid_app_technology_overview}.
...
\includegraphics[width=0.3\textwidth]{hybrid_app_architecture.pdf}
...
\begin{figure}\centering
\includegraphics[width=1.0\textwidth]{hybridni_aplikace_komunikace.pdf}
...
IBM Worklight je celá sada nástrojů pro vývoj mobilních aplikací. Základem je IBM Worklight Studio – IDE postavené na open-source platformě Eclipse, které krom klasických funkcí nabízí i možnost tvorby uživatelského rozhraní pomocí WYSIWYG editoru. Vývojář si pomocí funkce \uv{drag \& drop} přetahuje jednotlivé UI elementy (podporovány jsou HTML5, jQuery Mobile a Dojo Mobile) a modeluje si tím vzhled své aplikace.
